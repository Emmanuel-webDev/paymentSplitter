...
document.getElementById("connectWalletBtn").onclick = async () => {
  if (typeof window.ethereum == "undefined") {
    ...
    try {
      ...
      const tokenaddress = "0x89B50855Aa3bE2F677cD6303Cec089B5F319D72a"; // EURC contract address
...
async function displayContractBalance() {
  ...
  document.getElementById("Deposited").innerText = `: ${depositedBal} EURC`;
}

//Display wallet balance
async function displayWalletBalance() {
  const walBal = await tokenContract.balanceOf(await signer.getAddress());
  document.getElementById("WalletBalance").innerText = `${walBal / 1e6} EURC`;
}
...
document.getElementById("depositBtn").onclick = async () => {
  ...
  if (
    ...
    await showToast(
      `Minimum deposit amount is ${minDeposit / 1e6} EURC.`,
      "warning"
    );
    document.getElementById("depositWithdrawAmount").value = "";
    return;
...
document.getElementById("addRecipientBtn").onclick = async () => {
  ...
  listItem.textContent = `Address: ${newRecipient}, Amount: ${newAmount} EURC`;
  ...
  document.getElementById("totalSendingAmount").innerText = `${totalAmount} EURC`;
};
...
document.getElementById("resetInputBtn").onclick = async () => {
  recipients.length = 0;
  amounts.length = 0;
  document.getElementById("totalSendingAmount").innerText = `0 EURC`;
